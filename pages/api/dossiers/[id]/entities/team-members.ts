    ...
    try {
      ...
      if (method === 'GET') {
        if (entityId) {
          const result = await pool.query(
            `SELECT tm.*, r.name as role_name, r.description as role_description
             FROM ${TABLE_NAME} tm
      ...
      if (method === 'POST') {
        ...
        if (!name || !employmentType) {
          return res.status(400).json({ error: 'name e employmentType são obrigatórios' })
        }

        const result = await pool.query(
          `INSERT INTO ${TABLE_NAME} (clinic_id, dossier_id, name, role_id, employment_type, hours_per_week, notes, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, 'active')
           RETURNING *`,
          [clinicId, dossierId, name, roleId || null, employmentType, hoursPerWeek || null, notes || null],
        )
        
        // Marcar relatórios como stale
        const sectionCode = getSectionForEntity('team_members')
        if (sectionCode) {
