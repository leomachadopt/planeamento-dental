...
const TABLE_NAME = 'value_propositions'
...
export default async function handler(
  ...
  return authenticateToken(req as AuthRequest, res, async () => {
    try {
        ...
        if (entityId) {
          const result = await pool.query(
            `SELECT vp.*, cs.name as target_segment_name
             ...
             WHERE vp.id = $1 AND vp.clinic_id = $2 AND vp.dossier_id = $3`,
      ...
      if (method === 'POST') {
        const { title, description, targetSegmentId, notes } = req.body
        if (!title) {
          return res.status(400).json({ error: 'title é obrigatório' })
        }

        const result = await pool.query(
          `INSERT INTO ${TABLE_NAME} (clinic_id, dossier_id, title, description, target_segment_id, notes, status)
           VALUES ($1, $2, $3, $4, $5, $6, 'active')
           RETURNING *`,
          [clinicId, dossierId, title, description || null, targetSegmentId || null, notes || null],
        )
        
        // Marcar relatórios como stale
        const sectionCode = getSectionForEntity('value_propositions')
        if (sectionCode) {
      ...
      if (method === 'PUT' && entityId) {
        const { title, description, targetSegmentId, notes, status } = req.body
        ...
        if (targetSegmentId !== undefined) {
          updateFields.push(`target_segment_id = $${paramIndex++}`)
          updateValues.push(targetSegmentId)
        }
        if (notes !== undefined) {
