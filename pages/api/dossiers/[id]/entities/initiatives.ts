...
export default async function handler(
  ...
  return authenticateToken(req as AuthRequest, res, async () => {
    try {
      ...
      if (method === 'POST') {
        const { title, description, owner, timeframe, dependencyJson, notes } = req.body
        if (!title) {
          return res.status(400).json({ error: 'title é obrigatório' })
        }

        const result = await pool.query(
          `INSERT INTO ${TABLE_NAME} (clinic_id, dossier_id, title, description, owner, timeframe, dependency_json, notes, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'active')
           RETURNING *`,
          [clinicId, dossierId, title, description || null, owner || null, timeframe || null, dependencyJson ? JSON.stringify(dependencyJson) : null, notes || null],
        )
        
        // Marcar relatórios como stale
        const sectionCode = getSectionForEntity('initiatives')
        if (sectionCode) {
      ...
      if (method === 'PUT' && entityId) {
        const { title, description, owner, timeframe, dependencyJson, notes, status } = req.body
        ...
        if (dependencyJson !== undefined) {
          updateFields.push(`dependency_json = $${paramIndex++}`)
          updateValues.push(dependencyJson ? JSON.stringify(dependencyJson) : null)
        }
        if (notes !== undefined) {
