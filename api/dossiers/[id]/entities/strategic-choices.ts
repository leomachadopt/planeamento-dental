...
const TABLE_NAME = 'strategic_choices'
    ...
    try {
      ...
      if (method === 'GET') {
        if (entityId) {
          ...
          const result = await pool.query(
            `SELECT * FROM ${TABLE_NAME} WHERE clinic_id = $1 AND dossier_id = $2 ORDER BY priority, created_at`,
            [clinicId, dossierId],
          )
      ...
      if (method === 'POST') {
        const { title, rationale, priority, notes } = req.body
        if (!title) {
          return res.status(400).json({ error: 'title é obrigatório' })
        }

        const result = await pool.query(
          `INSERT INTO ${TABLE_NAME} (clinic_id, dossier_id, title, rationale, priority, notes, status)
           VALUES ($1, $2, $3, $4, $5, $6, 'active')
           RETURNING *`,
          [clinicId, dossierId, title, rationale || null, priority || null, notes || null],
        )
        
        // Marcar relatórios como stale
        const sectionCode = getSectionForEntity('strategic_choices')
        if (sectionCode) {
      ...
      if (method === 'PUT' && entityId) {
        const { title, rationale, priority, notes, status } = req.body
        ...
        if (rationale !== undefined) {
          updateFields.push(`rationale = $${paramIndex++}`)
          updateValues.push(rationale)
        }
